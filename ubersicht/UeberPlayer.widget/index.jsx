...
const options = {
  /* Widget size! */
  size: 'medium', // -> big (default) | medium | small | mini
  ...
  /* Adaptive colors! */
  adaptiveColors: false, // -> true (default) | false
...
const Wrapper = styled('div')`
  ...
  background: ${props =>
    props.bg !== undefined ? props.bg : 'rgba(29, 29, 31, 0.35)'};
...
const updateSongData = (output, error, previousState) => {
  ...
  // State controller
  if (!playing || appleMusicError) {
    // If player is paused or if online music is being played on Apple Music
    ...
    <UpdateText style={{ color }}>
      {updatePending ? (
        <span style={{ textDecoration: 'none', fontStyle: 'italic' }}>
          <b>Updating!</b> You might need to restart Ubersicht when finished...
          ({Math.floor(_updateTimer)})
        </span>
      ) : (
        <span onClick={() => dispatch({ type: 'PREPARE_UPDATE' })}>
  ...
  return (
    <BigPlayer>
      ...
      <Information>
        ...
        <Album className="small" color={tercaryColor}>
          {album}
        </Album>
      </Information>
    </BigPlayer>
...
const Mini = ({ state, dispatch }) => {
  ...
  return (
    <MiniPlayer>
      <MiniInfo>
        {options.miniArtwork && (
          <ArtworkImage artwork={artwork} wrapperClass="mini" />
        )}
        <MiniDetails>
      ...
      />
      {updateAvailable && (
        <UpdateNotif dispatch={dispatch} updatePending={updatePending} />
      )}
    </MiniPlayer>
  );
};
    ...
    return (
      ...
      >
        <ErrorWrapper>
          ...
          <p>
            Unfortunately, the widget is unable to access songs being streamed
            online through Apple Music.
          </p>
          <p>
            To fix this, please download your music to your library and play it
            locally from there.
          </p>
        </ErrorWrapper>
      </Wrapper>
